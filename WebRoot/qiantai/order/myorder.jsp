...
<html>
 ...
 <script type="text/javascript">
	        ...
	        {
	                 var url="<%=path %>/buy/orderDetail1?order_id="+order_id;
	                 var n="";
	                 var w="600px";
	                 var h="500px";
	                 var s="resizable:no;help:no;status:no;scroll:yes";
	        ...
	        {
	                 var url="<%=path %>/buy/updateZT?zt="+zt+"&orderid=" +order_id ;
							...
							<input value="明细" type="button" onclick="orderDetail(${order.id})"/>
							 ...
							 <c:if test="${order.zhuangtai=='未付款' && order.zhuangtai!='已取消'}">
							  ...
							  <c:if test="${order.zhuangtai=='已发货'}">
							   ...
							   <c:if test="${order.zhuangtai=='确认收货' && order.pjstate==0}">
							     <input value="评价" type="button" onclick="pingjia(${order.id})"/>
							 </c:if>
